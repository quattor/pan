...
import org.quattor.pan.tasks.Valid2Task;
...
public class Valid2Cache extends AbstractCache<Valid2Result> {
	...
	/**
	 * Creates a new <code>Valid2Cache</code> that has a default, initial size
	 * of 1000 entries.
	 * 
	 * @param compiler
	...
	public Valid2Cache(Compiler compiler, int size) {
		super(compiler, size);
	}

	@Override
	protected Task<Valid2Result> createTask(String objectName) {
		return new Valid2Task(compiler, objectName);
	}
