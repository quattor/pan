...
/**
 * Return the largest value from arguments
 *
 * @author jrha
 *
 */
final public class Max extends BuiltInFunction {

	private Max(SourceRange sourceRange, Operation... operations) throws SyntaxException {
		super("max", sourceRange, operations);
	}
	...
	public static Operation getInstance(SourceRange sourceRange, Operation... operations) throws SyntaxException {
		...
		if (operations.length != 2) {
			throw SyntaxException.create(sourceRange, MSG_TWO_ARGS_REQ, "max");
		}
		return new Max(sourceRange, operations);
	}
	...
	public Element execute(Context context) {
		...
		if ((args[0] instanceof LongProperty) && (args[1] instanceof LongProperty)) {
			...
			result = LongProperty.getInstance(Math.max(l1, l2));
			...
			result = DoubleProperty.getInstance(Math.max(d1, d2));

		} else {
			throw new EvaluationException("all arguments to max must be longs or doubles", getSourceRange(), context);
		}
