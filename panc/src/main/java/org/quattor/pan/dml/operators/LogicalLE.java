/*
 ...
 $HeadURL: https://svn.lal.in2p3.fr/LCG/QWG/panc/trunk/src/org/quattor/pan/dml/operators/LogicalLE.java $
 $Id: LogicalLE.java 3515 2008-07-31 13:20:05Z loomis $
 */

package org.quattor.pan.dml.operators;

import static org.quattor.pan.utils.MessageUtils.MSG_INVALID_ARGS_LE;
import static org.quattor.pan.utils.MessageUtils.MSG_MISMATCHED_ARGS_LE;
...
final public class LogicalLE extends AbstractOperation {
		...
		if (ops[0] instanceof Element && ops[1] instanceof Element) {
			...
			try {
				...
				throw new EvaluationException(MessageUtils
						.format(MSG_INVALID_ARGS_LE), sourceRange);
			} catch (EvaluationException ee) {
				throw SyntaxException.create(sourceRange, ee);
			}

		} else {
			result = new LogicalLE(sourceRange, ops);
		}
	...
	private static Element execute(SourceRange sourceRange, Property a,
		...
		if ((a instanceof NumberProperty) && (b instanceof NumberProperty)) {
			...
			throw new EvaluationException(MessageUtils
					.format(MSG_MISMATCHED_ARGS_LE), sourceRange);
		}
