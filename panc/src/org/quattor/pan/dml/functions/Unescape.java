/*
 ...
 $HeadURL: https://svn.lal.in2p3.fr/LCG/QWG/panc/trunk/src/org/quattor/pan/dml/functions/Unescape.java $
 $Id: Unescape.java 2799 2008-01-26 17:10:23Z loomis $
 */
...
/**
 * Unescape a string that was previously escaped with the <code>escape</code>
 * function.
 * 
 * @author loomis
 * 
...
final public class Unescape extends BuiltInFunction {
		...
		if (operations[0] instanceof Element) {
			...
			try {
				try {
					...
					String value = EscapeUtils.unescape(sp.getValue());
					result = StringProperty.getInstance(value);
				} catch (ClassCastException cce) {
					throw new EvaluationException(
							"unescape argument is not a string: "
			...
			result = new Unescape(sourceRange, operations);
		}
	...
	public Element execute(Context context) {
		...
		try {
			...
			return StringProperty.getInstance(EscapeUtils.unescape(s));

		} catch (EvaluationException ee) {
			throw ee.addExceptionInfo(sourceRange, context);

		} catch (ClassCastException cce) {
			throw new EvaluationException("unescape argument is not a string: "
