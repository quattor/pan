/*
 ...
 $HeadURL: https://svn.lal.in2p3.fr/LCG/QWG/panc/trunk/src/org/quattor/pan/dml/operators/LogicalGT.java $
 $Id: LogicalGT.java 3515 2008-07-31 13:20:05Z loomis $
 */

package org.quattor.pan.dml.operators;

import static org.quattor.pan.utils.MessageUtils.MSG_INVALID_ARGS_GT;
import static org.quattor.pan.utils.MessageUtils.MSG_MISMATCHED_ARGS_GT;
 ...
 * 
 */
final public class LogicalGT extends AbstractOperation {

	private static final long serialVersionUID = 8375790691781978705L;

	private LogicalGT(SourceRange sourceRange, Operation... operations) {
		...
		if (ops[0] instanceof Element && ops[1] instanceof Element) {
			...
			try {
				...
				throw new EvaluationException(MessageUtils
						.format(MSG_INVALID_ARGS_GT), sourceRange);
			} catch (EvaluationException ee) {
				throw SyntaxException.create(sourceRange, ee);
			}

		} else {
			result = new LogicalGT(sourceRange, ops);
		}
	...
	private static Element execute(SourceRange sourceRange, Property a,
		...
		if ((a instanceof NumberProperty) && (b instanceof NumberProperty)) {
			...
			result = BooleanProperty.getInstance(s1.compareTo(s2) > 0);

		} else {
			throw new EvaluationException(MessageUtils
					.format(MSG_MISMATCHED_ARGS_GT), sourceRange);
		}
